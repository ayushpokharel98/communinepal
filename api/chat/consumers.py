...
class ChatConsumer(AsyncJsonWebsocketConsumer):
    ...
    def can_join(self):
        return Conversation.objects.filter(id=self.conversation_id, members=self.user).exists()
    
    @database_sync_to_async
    def send_message(self, content, parent_id):
        MessageService.send_message(sender=self.user, conversation_id=self.conversation_id, content=content, parent_id=parent_id)
